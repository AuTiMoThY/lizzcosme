const IMGPATH = "./public/images/news/";
const newsTopData = [
    {
        "id": "1",
        "pic": `${IMGPATH}news1.jpg`,
        "tag": "保養知識",
        "time": "2021/04/26",
...
const newsList = Vue.createApp(app).mount("#newsList");
...
$(function () {
    ...
    var laboratorySwiper = new Swiper(".lizz_laboratory_pic", {
        slidesPerView: "auto",
        grabCursor: true,
        loop: true,
        autoplay: {
    ...
    let $news_card = $(".news_card");
    let $news_item = $(".news_item");
    $news_card.eq(0).addClass("js-show");
    $news_item.on("click", function () {
        let id = $(this).data("id");

        $news_card.each(function(i, e){
            if ($(e).data("id") == id) {
                $(e).addClass("js-show");
            } else {
